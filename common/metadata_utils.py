...
async def apply_meta_data_filter(
    ...
    元数据过滤器执行器，核心职责是：根据用户指定的模式（auto/semi_auto/manual），应用元数据过滤规则，返回过滤后的文档ID列表。
    根据三种模式（自动/半自动/手动）对文档元数据进行过滤，优先使用数据库下推执行，失败时回退到内存过滤，最终返回符合条件的文档ID列表。

    三种过滤模式
    模式	        含义	                                    使用场景
    auto	    LLM自动生成过滤条件	                    用户自然语言提问，由LLM自动理解并提取元数据过滤条件
    semi_auto	指定元数据Key，LLM只生成这些Key的过滤值	    用户明确要过滤哪些维度（如"时间"、"来源"），LLM只负责提取对应值
    manual	    手动指定完整过滤条件	                    用户/开发者明确知道要过滤什么，不需要LLM参与

    ┌─────────────────────────────────────────────────────────────────────────────────────┐
    │                              apply_meta_data_filter                                  │
    ├─────────────────────────────────────────────────────────────────────────────────────┤
    │                                                                                      │
    │  输入: meta_data_filter (过滤规则), question (用户问题), kb_ids (知识库范围)        │
    │                                                                                      │
    │  ┌─────────────────────────────────────────────────────────────────────────────┐   │
    │  │                    Step 1: 模式路由                                           │   │
    │  │                                                                              │   │
    ...
    │  │         ▼                 ▼                 ▼                               │   │
    │  │  ┌──────────────────────────────────────────────────────────────────────┐   │   │
    │  │  │                    gen_meta_filter (LLM生成过滤条件)                  │   │   │
    │  │  │       输入: 元数据Schema + 用户问题 + (可选)约束                      │   │   │
    │  │  │       输出: {"conditions": [...], "logic": "and/or"}                │   │   │
    │  │  └──────────────────────────────────────────────────────────────────────┘   │   │
    │  └─────────────────────────────────────────────────────────────────────────────┘   │
    │                                       │                                             │
    │                                       ▼                                             │
    ...
    │  │  ┌────────────────────────────────────────────────────────────────────┐   │   │
    │  │  │              路径A: 数据库下推 (Push-down)                           │   │   │
    │  │  │  条件: kb_ids存在 + ES/Infinity支持                                 │   │   │
    │  │  │  调用: DocMetadataService.filter_doc_ids_by_meta_pushdown()         │   │   │
    │  │  │  优势: 在数据库层过滤，高性能，减少网络传输                         │   │   │
    │  │  └────────────────────────────────────────────────────────────────────┘   │   │
    │  │                               │                                             │   │
    │  │                               ▼                                             │   │
    │  │  ┌────────────────────────────────────────────────────────────────────┐   │   │
    │  │  │              路径B: 内存过滤 (Fallback)                              │   │   │
    │  │  │  触发: 无kb_ids / 下推失败 / 数据库不支持                           │   │   │
    │  │  │  调用: meta_filter(_get_metas(), conditions, logic)                │   │   │
    │  │  │  优势: 兼容性好，不依赖特定后端                                     │   │   │
    │  │  └────────────────────────────────────────────────────────────────────┘   │   │
    │  └─────────────────────────────────────────────────────────────────────────────┘   │
    │                                       │                                             │
    ...
    │  │                    Step 3: 结果处理                                          │   │
    │  │                                                                              │   │
    │  │  • 有结果: 返回 doc_ids (list[str])                                         │   │
    │  │  • auto/semi_auto 无结果: 返回 None                                          │   │
    │  │  • manual 无结果: 返回 ["-999"] (哨兵值，表示无匹配)                        │   │
    │  └─────────────────────────────────────────────────────────────────────────────┘   │
    │                                                                                      │
    └─────────────────────────────────────────────────────────────────────────────────────┘
    ...
    # 执行流程：
    # 1. gen_meta_filter 分析问题 → 生成 {"conditions": [{"key": "year", "op": "eq", "value": "2024"}], "logic": "and"}
    # 2. 数据库下推执行过滤
    # 3. 返回符合条件的 doc_ids
    ...
    场景2：半自动模式（semi_auto）
    python
    # 用户明确要按 "year" 和 "category" 过滤
    result = await apply_meta_data_filter(
        meta_data_filter={
            "method": "semi_auto",
            "semi_auto": ["year", "category"]
        },
        question="最近一年的产品文档",
        chat_mdl=llm,
        kb_ids=["kb_001"]
    ...
    # 执行流程：
    # 1. LLM只从 "year" 和 "category" 中提取过滤值
    # 2. 生成条件 {"conditions": [{"key": "year", "op": "eq", "value": "2025"}, ...]}
    # 3. 执行过滤
    ...
    result = await apply_meta_data_filter(
        meta_data_filter={
            "method": "manual",
            "manual": [{"key": "status", "op": "eq", "value": "published"}],
            "logic": "and"
        },
        kb_ids=["kb_001"]
    )
    ...
    # 没有 kb_ids，走内存过滤
    result = await apply_meta_data_filter(
        meta_data_filter={"method": "manual", "manual": [{"key": "type", "op": "eq", "value": "report"}]},
        metas={"doc1": {"type": "report"}, "doc2": {"type": "memo"}}
    )

    # 执行流程：
    # 1. 无 kb_ids，无法下推
    # 2. 直接使用 _get_metas() 加载元数据
    # 3. 在内存中执行 meta_filter
    """
